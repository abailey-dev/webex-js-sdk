...
export default class Parser {
  ...
  static getNumbersOutOfRange(list, rangeStart, rangeEnd) {
    ...
    list.forEach((num) => {
      // if number is outside of specified range
  ...
  isValidLocus(newLoci) {
    ...
    const {isLoci} = Parser;
    const setStatus = function (status) { this.status = status; };
  ...
  static isSequenceEmpty(locus) {
    ...
    const hasEmptyEntries = !sequence.entries || !sequence.entries.length;
  ...
  processDeltaEvent() {
    ...
    if (this.debug) {
      ...
      // for full debugging.
      LoggerProxy.logger.info(`Locus-info:parser#processDeltaEvent --> Locus Debug: ${result}`);
    }
